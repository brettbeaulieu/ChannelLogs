...
export function FileTable({
  ...
  const [editingFilenameID, setEditingFilenameID] = useState<number | null>(
    null,
  )
  ...
  const handleSaveFilename = async () => {
    if (editingFilenameID && newFilename) {
      ...
      const response = await patchData(
        `${CHATFILES_URL}${editingFilenameID}/update_channel/`,
        formData,
      )
      if (!response.ok)
        throw new Error(
          `Failed to edit file with ID ${editingFilenameID}, Status ${response.status}`,
        )
      fetchFiles()
  ...
  const handleSaveNewChannel = async (
    ...
    const newChannelObj = channels.find(
    ...
    if (newChannelObj) {
      ...
      const response = await patchData(
        `${CHATFILES_URL}${chatfileID}/`,
        formData,
      )
      if (!response.ok)
        throw new Error(
          `Failed to edit file with ID ${oldChannelObj?.id}, Status ${response.status}`,
        )
      fetchChannels()
      fetchFiles()
    }
  ...
  const toggleAll = () => {
    setSelectedFiles(
      selectedFiles.size === 0
        ? new Set(chatFiles.map((file) => file.id))
        : new Set(),
    )
  }
  ...
  const buildTableRow = (chatfile: ChatFile) => {
    return (
      <Table.Tr
        key={chatfile.id}
        className={cx({ [styles.rowSelected]: selectedFiles.has(chatfile.id) })}
      >
        <Table.Td>
          <Checkbox
          ...
          {editingFilenameID === chatfile.id ? (
            <Group align="center">
              <TextInput
                ...
                style={{ marginRight: 8 }}
              />
              <Button onClick={handleSaveFilename} color="green" size="xs">
                Save
              </Button>
            ...
            <Group align="center">
              ...
              <Tooltip label="Edit Name">
                <Button
                  color="gray"
                  onClick={() =>
                    handleEditFilenameClick(chatfile.id, chatfile.filename)
                  }
                  variant="subtle"
                  size="xs"
                >
        ...
        <Table.Td hidden={!visibleColumns.actions} className={styles.fileCell}>
          {parseFormatDateTime(chatfile.uploaded_at)}
        </Table.Td>
        <Table.Td hidden={!visibleColumns.actions} className={styles.fileCell}>
          {JSON.stringify(chatfile.metadata)}
        </Table.Td>
        <Table.Td
        ...
        >
          <Box className={styles.actionGroup}>
            ...
            <Tooltip label="Delete File">
              <Button
                color="red"
                onClick={() => handleDelete(chatfile.id)}
                variant="subtle"
                size="xs"
              >
                <IconTrash size={20} />
    ...
    <Paper className={styles.mainPaper} withBorder>
      <Group mb="md">
        <Text className={styles.filesHeader}>
          Uploaded Files ({chatFiles.length})
        </Text>
        <VisibilityMenu
            ...
            <Table.Tr className={styles.tr}>
              <Table.Th className={styles.fileHeader}>
                <Checkbox
                  ...
                  indeterminate={
                    selectedFiles.size > 0 &&
                    selectedFiles.size !== chatFiles.length
                  }
                />
              </Table.Th>

              <Table.Th
                hidden={!visibleColumns.filename}
              ...
              <Table.Th
                hidden={!visibleColumns.is_preprocessed}
                className={styles.fileHeader}
                onClick={() => handleSort('is_preprocessed')}
              >
                Preprocessed
                {sortConfig.key === 'is_preprocessed' &&
                  sortIcon(sortConfig.direction)}
              </Table.Th>

              <Table.Th
                hidden={!visibleColumns.uploaded_at}
                className={styles.fileHeader}
              ...
              <Table.Th
                hidden={!visibleColumns.metadata}
